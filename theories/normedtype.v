...
Proof.
move=> realr; exists (r + 1); split; first by rewrite realD.
...
Proof.
move=> r0; exists (r - 1); split; first by rewrite realB//.
...
by rewrite realD//; split=> // x; apply: lt_le_trans; rewrite ltr_addl.
...
Lemma cvg_bounded_real {F : set (set V)} {FF : Filter F} (y : V) :
  ...
  \forall M \near +oo, \forall y' \near F, `|y'| < M.
Proof.
move=> /cvg_dist Fy; exists (`|y| + 1); rewrite realD//; split => // M.
...
- by exists r; split => // y ry; apply: rA; rewrite (le_trans ry)// lee_abs.
- exists (- r)%R; rewrite realN; split => // y; rewrite EFinN -lee_oppr => yr.
  by apply: rA; rewrite (le_trans yr)// -abseN lee_abs.
Qed.
...
- case: y => [y||//] /= in xy *.
  ...
  + by exists 0%R; split => // x /le_lt_trans; apply.
Qed.
...
- by move=> _; exists 0%R; split => // x; exact/lt_le_trans.
Qed.
...
have [->|x0] := eqVneq x 0; first by rewrite linear0 !normr0 mulr0.
have d0 : 0 < d by near: d; exact: nbhs_pinfty_gt.
...
rewrite realD ?realV// ?realE ?posnum_ge0; split => // r.
...
have r0  : 0 < r by near: r; exact: nbhs_pinfty_gt.
...
by rewrite ler_pmul //; near: r; exact: nbhs_pinfty_ge.
Unshelve. all: by end_near. Qed.
